  ...
  return {
    ...
    initEvent: function () {
      App.table = $("#table").DataTable({
        ...
        serverSide: true,
        ajax: {
          url: App.baseUrl + "kelompok_kelas/dataList",
          dataType: "json",
          type: "POST",
        },
        columns: [
      ...
      if ($("#form-kelompok_kelas").length > 0) {
        $("#save-btn").removeAttr("disabled");
        $("#form-kelompok_kelas").validate({
          rules: {
            kelas_id: {
              required: true,
            },
          ...
          submitHandler: function (form) {
            $.ajax({
              ...
              success: function (response) {
                ...
                setTimeout(function() {
                  window.location.href = App.baseUrl + "kelompok_kelas/";
              }, 1000);
              },
    ...
    initSend: function () {
      $.ajax({
        type: "POST",
        url: App.baseUrl + "kelompok_kelas/delete_data",
      });
    },
    importData: function () {
      $(document).ready(function () {
        $("#importForm").on("submit", function (event) {
          ...
          $.ajax({
            url: App.baseUrl + "kelompok_kelas/import_data",
            type: "POST",
            data: formData,
            processData: false,
            contentType: false,
