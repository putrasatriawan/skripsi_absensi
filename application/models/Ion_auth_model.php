...
class Ion_auth_model extends CI_Model
{
	...
	public function get_users_roles($where = array())
	{
		...
		$query = $this->db->get();
		if ($query->num_rows() > 0) {
			return $query->row(); // Menggunakan row() untuk mengambil satu baris
		}
		return FALSE;
	}
	...
	public function update($id, array $data)
	{
		...
		if (array_key_exists($this->identity_column, $data) || array_key_exists('password', $data) || array_key_exists('email', $data)) {
			if (array_key_exists('password', $data)) {
				if (!empty($data['password'])) {
					...
					// unset password so it doesn't affect database entry if no password is passed
					unset($data['password']);
				}
			}
		}
		...
		// Update user role in the 'users_roles' table
		if (array_key_exists('role_id', $data)) {
			// Check if role_id is in data
			$role_id = $data['role_id'];

			// First, remove all existing roles for the user
			$this->db->delete($this->tables['users_roles'], array('user_id' => $user->id));

			// Then, add the new role
			$this->db->insert($this->tables['users_roles'], array('user_id' => $user->id, 'role_id' => $role_id));
		}
