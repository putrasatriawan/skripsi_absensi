...
class Role extends Admin_Controller
{
	...
	public function create()
	{
		...
		if ($this->form_validation->run() === TRUE) {
			...
			$existing_role = $this->roles_model->checkNameAndDescription($name);
	...
	public function edit($id)
	{
		...
		if ($this->form_validation->run() === TRUE) {
			...
			$existing_role = $this->roles_model->checkNameAndDescriptionForUpdate($name, $id);

			if ($existing_role) {
				$response = array('status' => 'error', 'message' => 'Nama atau Deskripsi sudah digunakan atau tidak berubah, silakan gunakan yang berbeda.');
				header('Content-Type: application/json');
				echo json_encode($response);
				return;
			}
